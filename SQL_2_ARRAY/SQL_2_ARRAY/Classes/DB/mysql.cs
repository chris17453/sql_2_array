       ...
       public static List<Hashtable> getTables(string database){
        ...
        public static string createInsert(string database,string table,Hashtable data){
            ...
            foreach (String key in data.Keys) {
                switch(first) {
                    case true:  columns .Append(columnEnclosingLeft + dm.db.escapeString(key) + columnEnclosingRight);
                                values.Append("\""+escapeString(data[key].ToString())+"\"");
                                first = false;
                                break;
                    
                    case false: columns .Append(","+columnEnclosingLeft + dm.db.escapeString(key) + columnEnclosingRight);
                                values.Append(",\""+escapeString(data[key].ToString())+"\"");
                                break;
                    }
            }
